...
const CELL = 200, TILE_INSET = 12, MAJOR_EVERY = 4;
...
let tradeMode = false; let tradeJustId = null;
...
function drawGrid(){
  const dpr = DPR(); const vw = canvas.width / dpr, vh = canvas.height / dpr;
  const minor = 'rgba(255,0,255,0.10)';
  const major = 'rgba(255,0,255,0.22)';
...
function drawTiles(){
  ...
  ctx.strokeStyle = pulseIds.has(t.id) ? 'rgba(255,0,255,0.55)' : 'rgba(255,0,255,0.28)';
  ctx.lineWidth = pulseIds.has(t.id) ? 2 : 1; ctx.stroke(); ctx.restore();
    // Thumb
    ...
    const name = t.name || 'Untitled'; ctx.font = `${Math.max(12, 12*scale)}px system-ui, sans-serif`;
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.shadowColor = 'rgba(0,0,0,0.6)'; ctx.shadowBlur = 4; ctx.shadowOffsetY = 1;
    ctx.fillStyle = '#fff'; ctx.fillText(name, dx + 10*scale, dy + h - Math.max(18*scale, 14)); ctx.shadowBlur = 0; ctx.shadowColor = 'transparent';
    ctx.restore();
  }
  // Empty state but still draw grid—do nothing else
  if (!tiles.length) return;
  // Expanded overlay (static preview frame; preview uses community.js overlay)
  if (expandedTile) {
    ...
    ctx.fillStyle = 'rgba(20,20,20,0.98)'; ctx.fill(); ctx.strokeStyle = 'rgba(255,0,255,0.28)'; ctx.lineWidth = 1; ctx.stroke();
    ...
    ctx.fillStyle = '#fff'; ctx.font = '16px system-ui, sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic'; ctx.shadowColor = 'rgba(0,0,0,0.7)'; ctx.shadowBlur = 8; ctx.shadowOffsetY = 2; ctx.fillText(expandedTile.name || 'Untitled', sX+14, sY+sH-16); ctx.shadowBlur = 0;
    const btnW = 84, btnH = 30, by = sY + sH - btnH - 12, bxOpen = sX + sW - btnW - 12, gap = 8, bxPreview = bxOpen - gap - btnW;
    expandedTile.btnPreview = { x: bxPreview, y: by, w: btnW, h: btnH }; expandedTile.btnOpen = { x: bxOpen, y: by, w: btnW, h: btnH };
    drawButton(expandedTile.btnPreview, 'Preview'); drawButton(expandedTile.btnOpen, 'Add');
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (t) {
    if (tradeMode && pulseIds.size && !pulseIds.has(t.id)) { // in trade mode, only allow choosing from highlighted tiles
...
canvas.addEventListener('click', (e) => {
  ...
  const { x, y, w, h } = t.overlayRect; const bxPrev = { ...t.btnPreview }, bxOpen = { ...t.btnOpen };
  if (sx >= bxPrev.x && sx <= bxPrev.x + bxPrev.w && sy >= bxPrev.y && sy <= bxPrev.y + bxPrev.h) {
    window.dispatchEvent(new CustomEvent('community:preview', { detail: { id: t.id } })); return;
  }
  if (sx >= bxOpen.x && sx <= bxOpen.x + bxOpen.w && sy >= bxOpen.y && sy <= bxOpen.y + bxOpen.h) {
    ...
    (async () => { const rec = await communityApi.getCommunityScene(t.id).catch(()=>null); if (rec) {
      const local = await import('./local-store.js'); await local.saveScene({ name: rec.name, json: rec.json, thumb: rec.thumb });
      if (tradeMode) { try { sessionStorage.setItem('sketcher:lastTradePick', t.id); } catch {}
        window.location.href = './columbarium.html';
      } else { alert('Added to your collection!'); }
    } })();
    return;
  }
...
async function loadTiles(){
  const items = await communityApi.listLatestCommunity(10).catch(()=>[]);
  // Fixed 2 rows x 5 cols layout; center spacing like personal
  const cols = 5, rows = 2; const spacing = CELL * 1.4;
  tiles = items.map((it, idx) => {
    const r = Math.floor(idx / cols); const c = idx % cols;
    return { id: it.id, name: it.name, x: (c * spacing) + CELL/2, y: (r * spacing) + CELL/2, w: CELL, h: CELL, thumb: it.thumb || null };
  });
  for (const t of tiles) {
    if (t.thumb && !thumbCache.has(t.id)) { const img = new Image(); img.onload = () => { thumbCache.set(t.id, img); draw(); }; img.src = t.thumb; }
  }
...
// Parse trade intent
try { const qs = new URLSearchParams(location.search); tradeMode = (qs.get('trade') === '1'); tradeJustId = qs.get('just') || null; } catch {}
resize(); loadTiles().then(()=>draw());
